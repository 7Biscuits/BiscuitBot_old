...
client.warnings = {}
filters = [
    "Fuck", "fuck", "bitch", "Bitch", "Asshole", "Mother fucker", "asshole"
]
...
async def warn(ctx, member: discord.Member = None, *, reason=None):
	if member is None:
		return await ctx.send(
		    "The provided member could not be found or you didn't to provide one."
		)
	...
	if reason is None:
		...
		async with aiofiles.open(f"{member.guild.id}.txt", mode="a") as file:
			...
			await ctx.send(
			    f'{member.mention} has been warned successfully, Reason: {reason}. They now have {count} warnings.'
			)
			await member.send(
			    f'You have been warned in {ctx.guild.name} by {ctx.author.name}, Reason: {reason}. You now have {count} warnings '
			)
...
async def checkwarn(ctx, member: discord.Member = None):
	if member is None:
		return await ctx.send(
		    "The provided member could not be found or you didn't provide one."
		)
		...
		try:
			i = 1
			for admin_id, reason in client.warnings[ctx.guild.id][
			    member.id][1]:
...
async def Ban(ctx, member: discord.Member, *, reason='No reason provided'):
	...
	await ctx.send(
	    f'{member} has been banned from the server uccessfully, Reason: ' +
	    reason)
	await member.send(
	    f'You have been banned from {ctx.guild.name} by {ctx.author.name}, Reason: '
	    + reason)
...
async def unban(ctx, *, member: discord.Member):
	...
	for banned_entry in banned_users:
		...
		if (user.name, user.discriminator) == (member_name, member_disc):
			...
			await member.send(
			    f'you have been unbanned from {ctx.guild.name} by {ctx.author.name}'
			)
			return
...
async def message(ctx, member: discord.Member, *, message="None"):
	await member.send(
	    f'{ctx.author.name} sent a message for you. MESSAGE: {message}')
...
async def mention(ctx, member: discord.Member, *, reason='None'):
	await ctx.send(
	    f'{ctx.author.name} has mentioned {member.mention} in {ctx.guild.name}, Reason: {reason}'
	)
	await member.send(
	    f'{ctx.author.name} mentioned you in {ctx.guild.name}, Reason: {reason}'
	)
...
async def mute(ctx, member: discord.Member, *, reason="No Reason Provided"):
	...
	await member.send(
	    f" You have been muted from: {ctx.guild.name} by {ctx.author.name} Reason: {reason}"
	)
